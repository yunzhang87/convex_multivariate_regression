## functions solves 1/(2n) tr( Omega (Y-XC)^T (Y-XC) ) + lamC * \|C\|_{1,2} 
## and its TLP version ##

# source("first_order_opt.R")
# para = [C] \in R^{q*p}, which stacks rows C into a vector #
grad_g_full = function(para, X, Y, XtX, OmegaYtX, YtY, Omega){
	n = dim(X)[1]; p = dim(XtX)[1]; q = dim(YtY)[1];
  ### set para to be rows of C ###
	C = t(matrix(para, q, p))
	as.vector(crossprod(Omega, crossprod(C, XtX)) - OmegaYtX)
}

obj_g_full = function(para, X, Y, Omega){
	...
	C = t(matrix(para, q, p))
	tmp = crossprod(Y - X%*%C)
	obj_g = .5*sum(tmp*Omega)
	obj_g
}

obj_full = function(para, X, Y, Omega, lamC){
	norm_vec <- function(x) sqrt(sum(x^2))
	q = dim(Y)[2]
	p = dim(X)[2]
	C = t(matrix(para, q, p))
	penalty = lamC * sum(apply(C,1,norm_vec))
	(obj_g_full(para, X, Y, Omega) + penalty)
}

obj_full_nonconvex = function(para, X, Y, Omega, lamC, tau) {
	norm_vec <- function(x) sqrt(sum(x^2))
...
rblasso_alt <- function(X, Y, lamC, Omega, C0=NULL, 
	max.iter=1e3, fast=T, eps=1e-3, nonconvex=F, 
	tau=NULL, warm.start=F)
{
	n = dim(X)[1]; p = dim(X)[2]; q = dim(Y)[2];
	X = X /sqrt(n); Y = Y/sqrt(n);
	YtX = crossprod(Y,X)
	OmegaYtX = crossprod(Omega, YtX)
	...
	obj_g = function(para) obj_g_full(para, X, Y, Omega)
	grad_g = function(para) grad_g_full(para, X, Y, XtX, OmegaYtX, YtY, Omega)
	...
	for (i in seq_along(lamC)) {
		...
		if (nonconvex) {
			...
			obj = function(para) obj_full_nonconvex(para, X, Y, Omega, lam.c, tau)
		} else {
			prox_h = function(y, t) prox_h_full(y, t, lam.c, p, q)
			obj = function(para) obj_full(para, X, Y, Omega, lam.c)
		}
		if (warm.start) {
			if (i == 1 & j == 1) {
				if (fast) { 
